...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #fantomcoinTEST\r");
            Send(hSocket, "WHO #fantomcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #fantomcoin\n");
            // randomly join #fantomcoin00-#fantomcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #fantomcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #fantomcoin%02d\r", channel_number).c_str());
        };
