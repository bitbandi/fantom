        ...
        if(pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (!fIsInitialDownload && pindex != NULL)
                {
                    ...
                    CScript payee;
                    CTxIn vin;
                    if(!blanknodePayments.GetBlockPayee(pindexBest->nHeight+1, payee, vin) || payee == CScript()){                        
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if(!fLiteMode){
        if (!fImporting && !fReindex && pindexBest->nHeight > Checkpoints::GetTotalBlocksEstimate()){
            ...
            CScript payee;
            CTxIn vin;
            if(blanknodePayments.GetBlockPayee(pindexBest->nHeight, payee, vin)){
                //UPDATE BLANKNODE LAST PAID TIME
                CBlanknode* psn = snodeman.Find(vin);
                if(psn != NULL) psn->nLastPaid = GetAdjustedTime(); 

                LogPrintf("ProcessBlock() : Update Blanknode Last Paid Time - %d\n", pindexBest->nHeight);
            }
