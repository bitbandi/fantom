...
FantomGUI::FantomGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
    ...
    nWeight(0) {
    resize(1000, 600);
    setWindowTitle(tr("Fantom") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/fantom"));
    setWindowIcon(QIcon(":icons/fantom"));
#else
    ...
    p.setColor(QPalette::Text, QColor(248, 248, 255));
    setPalette(p);
    ...
    fantomMarket = new FantomMarket(this);

    buysPage = new BuysPage(this);

    sellsPage = new SellsPage(this);

    blanknodeManagerPage = 0;
    ...
    centralStackedWidget->addWidget(fantomMarket);
...
void FantomGUI::createActions() 
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Fantom address"));
    ...
    statisticsAction->setToolTip(tr("FNX PoW/PoS Statistics"));
    ...
    fantomMarketAction = new QAction(QIcon(":/icons/fantom"), tr("&FantomMarket"), this);
    fantomMarketAction->setToolTip(tr("Browse the Market."));
    fantomMarketAction->setCheckable(true);
#ifdef Q_OS_MAC
    fantomMarketAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_9));
#else
    fantomMarketAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
#endif
    tabGroup->addAction(fantomMarketAction);


    buysPageAction = new QAction(QIcon(":/icons/fantom"), tr("&Market Purchases"), this);
    buysPageAction->setToolTip(tr("Show my FantomMarket Buys."));
    ...
    sellsPageAction = new QAction(QIcon(":/icons/fantom"), tr("&Market Sells"), this);
    sellsPageAction->setToolTip(tr("Show my FantomMarket Sells."));
    sellsPageAction->setCheckable(true);
    tabGroup->addAction(sellsPageAction);

    blanknodeManagerAction = new QAction(QIcon(":/icons/fantom"), tr("&Zerosend"), this);
    blanknodeManagerAction->setToolTip(tr("Show Blanknodes status and configure your nodes."));
    blanknodeManagerAction->setCheckable(true);
    tabGroup->addAction(blanknodeManagerAction);
    ...
    connect(fantomMarketAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(fantomMarketAction, SIGNAL(triggered()), this, SLOT(gotoFantomMarket()));
    ...
    connect(blanknodeManagerAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(blanknodeManagerAction, SIGNAL(triggered()), this, SLOT(gotoBlanknodeManagerPage()));
    ...
    aboutAction = new QAction(tr("&About Fantom"), this);
    aboutAction->setToolTip(tr("Show information about Fantom"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for Fantom"));
    optionsAction->setMenuRole(QAction::PreferencesRole);

    toggleHideAction = new QAction(QIcon(":/icons/fantom"), tr("&Show / Hide"), this);
...
void FantomGUI::createToolBars() 
{
    QLabel* header = new QLabel();
    header->setMinimumSize(48, 48);
    header->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    header->setPixmap(QPixmap(":/icons/fantom"));
...
void FantomGUI::setClientModel(ClientModel *clientModel) 
{
    if(!fOnlyTor) 
    {
#ifdef USE_NATIVE_I2P
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/fantom_testnet"));
            setWindowIcon(QIcon(":icons/fantom_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/fantom_testnet"));
#endif

            if(trayIcon) 
            {
                trayIcon->setToolTip(tr("Fantom client") + QString(" ") + tr("[testnet]"));
...
void FantomGUI::setWalletModel(WalletModel *walletModel) 
{
...
void FantomGUI::setMessageModel(MessageModel *messageModel)
{
...
void FantomGUI::createTrayIcon() {
    ...
    trayIcon->setToolTip(tr("Fantom client"));
...
#ifndef Q_OS_MAC
void FantomGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason) 
{
...
void FantomGUI::unlockIconClicked() 
{
    if(!walletModel) 
    {
        return;
...
void FantomGUI::lockIconClicked() 
{
    if(!walletModel) 
    {
        return;
...
#ifdef USE_NATIVE_I2P
void FantomGUI::showGeneratedI2PAddr(const QString& caption, const QString& pub, const QString& priv, const QString& b32, const QString& configFileName) {
...
#ifdef USE_NATIVE_I2P
void FantomGUI::setNumI2PConnections(int count) 
{
    QString i2pIcon;
    ...
    labelI2PConnections->setToolTip(tr("%n active connection(s) to I2P-Fantom network", "", count));
}
#endif

void FantomGUI::setNumConnections(int count) 
{
    QString icon;
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Fantom network", "", count));
}

void FantomGUI::setNumBlocks(int count) 
{
    if(!clientModel)
        return;
...
void FantomGUI::message(const QString &title, const QString &message, bool modal, unsigned int style, bool *ret) 
{   
    
    QString strTitle = tr("Fantom") + " - ";
...
void FantomGUI::changeEvent(QEvent *e) 
{
...
void FantomGUI::closeEvent(QCloseEvent *event) 
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel&& clientModel->getOptionsModel())
         {
...
void FantomGUI::askFee(qint64 nFeeRequired, bool *payFee) 
{
    if (!clientModel || !clientModel->getOptionsModel()) 
    {
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
                            ...
                            "Do you want to pay the fee?").arg(FantomUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void FantomGUI::incomingTransaction(const QModelIndex & parent, int start, int end) 
{
    ...
    if(!clientModel->inInitialBlockDownload()) {
        ...
        notificator->notify(Notificator::Information,
                            ...
                            tr("Date: %1\n"
                               ...
                               "Address: %4\n")
                            .arg(date)
                            .arg(FantomUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                            .arg(type)
                            .arg(address), icon);
    }
}

void FantomGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void FantomGUI::gotoOverviewPage() 
{
...
void FantomGUI::gotoHistoryPage() 
{
...
void FantomGUI::gotoAddressBookPage() 
{
...
void FantomGUI::gotoReceiveCoinsPage() 
{
...
void FantomGUI::gotoSendCoinsPage() 
{
...
void FantomGUI::gotoSignMessageTab(QString addr) {
...
void FantomGUI::gotoVerifyMessageTab(QString addr) 
{
...
void FantomGUI::gotoBlockBrowser() 
{
...
void FantomGUI::gotoStatisticsPage() 
{
...
void FantomGUI::gotoMessagePage()
{
...
void FantomGUI::gotoFantomMarket()
{
    //FantomMarketAction->setChecked(true);
    //centralStackedWidget->setCurrentWidget(fantomMarket);
...
void FantomGUI::gotoBlanknodeManagerPage() 
{

    if(!blanknodeManagerPage)
    {
        blanknodeManagerPage = new BlanknodeManager(this);
        centralStackedWidget->addWidget(blanknodeManagerPage);
    }

    blanknodeManagerAction->setChecked(true); 
    centralStackedWidget->setCurrentWidget(blanknodeManagerPage);

    exportAction->setEnabled(false);
    disconnect(exportAction, SIGNAL(triggered()), 0, 0);
}

void FantomGUI::dragEnterEvent(QDragEnterEvent *event) 
{
    // Accept only URIs
...
void FantomGUI::dropEvent(QDropEvent *event) 
{
    if(event->mimeData()->hasUrls()) {
        ...
        if (nValidUrisFound) {
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Fantom address or malformed URI parameters."));
        }
    }
...
void FantomGUI::handleURI(QString strURI) 
{
    // URI has to be valid
...
void FantomGUI::setEncryptionStatus(int status) 
{
...
void FantomGUI::changePassphrase() 
{
...
void FantomGUI::showNormalIfMinimized(bool fToggleHidden) 
{
...
void FantomGUI::toggleHidden() 
{
    showNormalIfMinimized(true);
}

void FantomGUI::updateWeight() 
{
    if (!pwalletMain) {
        return;
    }
...
void FantomGUI::updateStakingIcon() 
{
    updateWeight();
    ...
    if (nLastCoinStakeSearchInterval && nWeight) {
        ...
        unsigned nEstimateTime = 28 * nNetworkWeight / nWeight;
...
WId FantomGUI::getMainWinId() const 
{
    return winId();
}
