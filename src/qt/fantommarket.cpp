...
FantomMarket::FantomMarket(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FantomMarket)
{
    ui->setupUi(this);
    subscribeToCoreSignals();

    ui->fantomMarketHeaderLabel->setPixmap(QPixmap(":/images/fantommarket"));
    updateCategories();
...
FantomMarket::~FantomMarket()
{
    unsubscribeFromCoreSignals();
    delete ui;
}

void FantomMarket::on_categoriesListWidget_currentItemChanged(QListWidgetItem* current, QListWidgetItem* previous)
{
	...
	QTableWidgetItem* untilItem = new QTableWidgetItem(QString::fromStdString(DateTimeStrFormat(p.listing.nCreated + (7 * 24 * 60 * 60))));
        QTableWidgetItem* vendorItem = new QTableWidgetItem(QString::fromStdString(CFantomAddress(p.listing.sellerKey.GetID()).ToString()));
...
void FantomMarket::updateCategories()
{
...
void FantomMarket::updateCategory(QString category)
{
    if(category == "")
	return;
...
static void NotifyMarketCategory(FantomMarket *page, std::string category)
{
...
static void NotifyListingCancelled(FantomMarket *page)
{
    
}

void FantomMarket::subscribeToCoreSignals()
{
...
void FantomMarket::unsubscribeFromCoreSignals()
{
...
void FantomMarket::on_tableWidget_itemSelectionChanged()
{
...
void FantomMarket::on_buyButton_clicked()
{
...
void FantomMarket::on_viewDetailsButton_clicked()
{
    QItemSelectionModel* selectionModel = ui->tableWidget->selectionModel();
    QModelIndexList selected = selectionModel->selectedRows();
    if(selected.count() == 0)
...
void FantomMarket::on_copyAddressButton_clicked()
{
    QItemSelectionModel* selectionModel = ui->tableWidget->selectionModel();
    QModelIndexList selected = selectionModel->selectedRows();
    if(selected.count() == 0)
