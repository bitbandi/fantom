...
{
    LogPrintf("Fantom Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running Fantom Miner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("Fantom Miner terminated\n");
        throw;
    }
}
