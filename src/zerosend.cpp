...
bool CZerosendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nLowestDenom = ZEROSEND_COLLATERAL + zeroSendDenominations[zeroSendDenominations.size() - 1]*5;
    ...
    if(!sessionFoundBlanknode){
        ...
        double fFantomSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Zerosend for %f FNX CENT - sessionTotalValue %d\n", fFantomSubmitted, sessionTotalValue);
...
int CZerosendPool::GetDenominations(const std::vector<CTxOut>& vout){
...
int CZerosendPool::GetDenominationsByAmounts(std::vector<int64_t>& vecAmount){
    CScript e = CScript();
